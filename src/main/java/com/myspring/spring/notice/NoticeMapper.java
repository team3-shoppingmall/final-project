...
public interface NoticeMapper {
	...
	// 전체 개수 가져오기
	@Select("select count(*) from noticetable where title like CONCAT('%',#{searchWord},'%')")
	public int getCount(@Param("searchWord") String searchWord);

	// 공지사항 목록 출력
	@Select("select * from noticetable where title like CONCAT('%', #{searchWord}, '%') order by noticeno desc limit #{start}, #{perPage}")
